...
# Suppress warnings from pyannote and torchcodec
# Why these warnings appeared:
# 1. **torchcodec warning**: pyannote tries to load torchcodec to decode audio, 
#    but fails. This isn't a problem because the script already uses soundfile 
#    as a workaround (lines 119-129)
# 2. **SyntaxWarnings**: Issues in the pyannote library code itself (regex `\s+`
#    should be `\\s+`). These are warnings from the library, not from our code.                                                              
import warnings
warnings.filterwarnings("ignore", category=UserWarning, module="pyannote.audio.core.io")
warnings.filterwarnings("ignore", category=SyntaxWarning, module="pyannote.database")
...
def record_and_transcribe_with_diarization(model_size="base", segment_minutes=5):
    ...
    if model_size in ["tiny", "base", "small", "medium", "large"]:
        ...
        mlx_model_path = "mlx-community/whisper-large-v3-turbo"
    else:
...
if __name__ == "__main__":
    ...
    model = input("\nWhisper model (tiny/base/small/medium/large/turbo) [medium]: ").strip() or "medium"
